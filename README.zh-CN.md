...
| `ZOTERO_ID` | `1234567` | 使用 Zotero 文库时填写。在 [Zotero Settings](https://www.zotero.org/settings/security#applications) 的 `Create new private key` 按钮附近查看。可参考 [Zotero API Key Guide](https://oeysan.github.io/c2z/articles/zotero_api.html#step-1-locate-zotero-settings)。 | :closed_book:`推荐` |
| `ZOTERO_KEY` | `zotero-api-key` | Zotero API key。在 [Zotero Settings](https://www.zotero.org/settings/security#applications) 创建一个至少有 Read 权限的新 key。可参考 [Zotero API Key Guide](https://oeysan.github.io/c2z/articles/zotero_api.html#step-2-create-a-key)。 | :closed_book:`推荐` |
...
> [!NOTE]
> **修改每日邮件发送时间：**
> 默认计划任务在 UTC+8 的 09:11 运行，也就是 UTC 的 01:11。这个时间由 `cron` 表达式控制，默认是：`11 1 * * *`。
>
...
如果有问题、建议或使用体验，可以在仓库中 [创建 issue](https://github.com/nehSgnaiL/paper-daily-feed/issues/new)。
